...
async function cleanupImages() {
  ...
  try {
    ...
    // Create a set of all referenced images
    const referencedImages = new Set([
      ...wins.map((w) => stripNumberFromFilename(w.us)),
      ...wins.map((w) => stripNumberFromFilename(w.opponent)),
      ...losses.map((l) => stripNumberFromFilename(l.us)),
      ...losses.map((l) => stripNumberFromFilename(l.opponent)),
    ]);

    // Iterate through each file in the images directory
    for (const file of files) {
      // If the file (after stripping the number) is not in the referencedImages set, delete it
